...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    if is_main_process:
        ...
        logging.info(f"Local train metrics JSONL: {cfg.output_dir / 'train_metrics.jsonl'}")
        if cfg.env is not None:
            logging.info(f"Local eval metrics JSONL: {cfg.output_dir / 'eval_metrics.jsonl'}")
        if cfg.env is not None:
    ...
    for _ in range(step, cfg.steps):
        ...
        if is_log_step:
            ...
            local_log_dict = {"mode": "train", "step": step, **train_tracker.to_dict()}
            if output_dict:
                local_log_dict.update(output_dict)
            if sample_weighter is not None:
                weighter_stats = sample_weighter.get_stats()
                local_log_dict.update({f"sample_weighting/{k}": v for k, v in weighter_stats.items()})
            _append_jsonl(cfg.output_dir / "train_metrics.jsonl", local_log_dict)
            if wandb_logger:
        ...
        if cfg.env and is_eval_step:
            if is_main_process:
                ...
                eval_info_for_json = _json_safe(eval_info)
                ...
                _append_jsonl(
                    cfg.output_dir / "eval_metrics.jsonl",
                    {
                        "mode": "eval",
                        "step": step,
                        **eval_tracker.to_dict(),
                        "eval_info": eval_info_for_json,
                    },
                )
                if wandb_logger:
