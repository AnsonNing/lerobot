...
from .configuration_streaming_flow import StreamingFlowV4Config
...
class StreamingFlowPolicy(CLIPStreamingFlowPolicy):
    """SFP with token-level CLIP grounding and an AdaLN-Zero transformer velocity expert."""

    config_class = StreamingFlowV4Config
    name = "streaming_flow_v4"

    def __init__(self, config: StreamingFlowV4Config, **kwargs):
...
class StreamingFlowModel(CLIPStreamingFlowModel):
    ...
    def __init__(self, config: StreamingFlowV4Config):
    ...
    def _prepare_token_conditioning(self, batch: dict[str, Tensor]) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        if self.text_encoder is not None:
            if OBS_LANGUAGE_TOKENS not in batch or OBS_LANGUAGE_ATTENTION_MASK not in batch:
                raise ValueError(
                    "StreamingFlowV4 requires tokenized task text when CLIP text conditioning is enabled."
                )
        ...
        if not cond_feats or batch_size is None:
            raise ValueError("StreamingFlowV4 received no conditioning features.")
    ...
    def _predict_frequency(self, raw_cond: Tensor, clamp: bool) -> Tensor:
        if not self.config.sfp_use_adaptive_freq:
            return torch.ones(raw_cond.shape[0], device=raw_cond.device, dtype=torch.float32)
        freq = self.velocity_model.granularity_predictor(raw_cond.float())
        if clamp:
...
class CLIPPatchTokenImageEncoder(nn.Module):
    """Frozen CLIP vision encoder returning pooled features and compressed patch tokens."""
...
class AdaLNZeroCrossAttentionBlock(nn.Module):
    """Transformer decoder block with pooled-context AdaLN-Zero modulation."""
    ...
    def forward(self, x: Tensor, memory: Tensor, memory_padding_mask: Tensor, condition: Tensor) -> Tensor:
        ...
        x = x + gate_self[:, None] * self.dropout(self_out)
        ...
        x = x + gate_cross[:, None] * self.dropout(cross_out)
        mlp_input = modulate(self.norm_mlp(x), shift_mlp[:, None], scale_mlp[:, None])
        x = x + gate_mlp[:, None] * self.dropout(self.mlp(mlp_input))
        return x


class AdaptiveStreamingFlowTransformer(nn.Module):
    """Token-grounded transformer predicting SFP velocity with AdaLN-Zero conditioning."""

    def __init__(self, config: StreamingFlowV4Config, global_cond_dim: int):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        for block in self.blocks:
            hidden = block(hidden, memory, memory_padding_mask, condition)
