...
class StreamingFlowPolicy(PreTrainedPolicy):
    ...
    def update(self) -> None:
        ...
        with torch.no_grad():
            ...
            if self.ema_model.text_encoder is not None:
                self.ema_model.text_encoder.load_state_dict(self.model.text_encoder.state_dict())
    ...
    def _initial_rollout_action(self, queued_batch: dict[str, Tensor]) -> tuple[Tensor | None, str]:
        ...
        init_action = torch.zeros(
            (images.shape[0], 1, self.config.action_feature.shape[0]),
            device=images.device,
            dtype=images.dtype,
        )
        fixed_action = torch.as_tensor(
            self.NOTEBOOK_INITIAL_ACTION,
            device=images.device,
            dtype=images.dtype,
        )
        if fixed_action.numel() != self.config.action_feature.shape[0]:
            return init_action, "zero_action"

        init_action[:, 0, : fixed_action.numel()] = fixed_action
        if not self._has_action_stats():
            if fixed_action.abs().max() <= 1.0:
                return init_action, "notebook_fixed_center"
            return torch.zeros_like(init_action), "zero_action"
        return self._normalize_action(init_action), "notebook_fixed_center"
    ...
    def _predict_action_chunk_and_state(
        ...
        # The task instruction is constant within an episode and should not be
        # temporally queued with image/state observations.
        for key in (OBS_LANGUAGE_TOKENS, OBS_LANGUAGE_ATTENTION_MASK):
            if key in batch:
                queued_batch[key] = batch[key]

        # Match notebook rollout: first chunk starts from [256, 256], later
        # chunks continue from the previous final action state.
...
class StreamingFlowModel(nn.Module):
    def __init__(self, config: StreamingFlowConfig):
        ...
        if config.image_features:
            ...
            if config.use_separate_rgb_encoder_per_camera:
                encoder_cls = CLIPTemporalImageEncoder if config.sfp_use_clip_image_conditioning else TemporalImageEncoder
                self.rgb_encoder = nn.ModuleList(
                    [encoder_cls(config) for _ in range(num_images)]
                )
    ...
    def _prepare_global_conditioning(self, batch: dict[str, Tensor]) -> tuple[Tensor, Tensor]:
        ...
        if self.text_encoder is not None:
            if OBS_LANGUAGE_TOKENS not in batch or OBS_LANGUAGE_ATTENTION_MASK not in batch:
                raise ValueError(
                    "StreamingFlowV3 requires tokenized task text when CLIP text conditioning is enabled. "
            ...
            text_feats = text_feats.unsqueeze(1).expand(-1, self.config.n_obs_steps, -1)
            cond_feats.append(text_feats.flatten(start_dim=1))
    ...
    def _predict_frequency(self, raw_cond: Tensor, clamp: bool) -> Tensor:
        ...
        freq = self.velocity_model.granularity_predictor(raw_cond.to(dtype=torch.float32))
        if clamp:
            freq = torch.clamp(freq, min=self.config.sfp_freq_min, max=self.config.sfp_freq_max)
        return freq
    ...
    def integrate_actions(
        ...
        freq = self._predict_frequency(raw_cond, clamp=self.config.sfp_clamp_freq_during_eval)
    ...
    def compute_loss(
        ...
        raw_cond, normalized_cond = self._prepare_global_conditioning(batch)
        freq = self._predict_frequency(raw_cond, clamp=self.config.sfp_clamp_freq_during_training)
...
class CLIPTemporalImageEncoder(nn.Module):
    """Frozen CLIP vision encoder fused over the observation window."""
    ...
    def __init__(self, config: StreamingFlowConfig):
        ...
        if config.clip_image_crop_shape is not None:
            ...
            if config.clip_image_crop_is_random:
                self.maybe_random_crop = torchvision.transforms.RandomCrop(config.clip_image_crop_shape)
            else:
    ...
    def forward(self, obs: Tensor) -> Tensor:
        ...
        if self.config.sfp_freeze_clip:
            with torch.no_grad():
                features = self.model(pixel_values=obs, output_hidden_states=False).last_hidden_state[:, 0]
        else:
            features = self.model(pixel_values=obs, output_hidden_states=False).last_hidden_state[:, 0]
...
class CLIPTextConditionEncoder(nn.Module):
    """Frozen CLIP text encoder with a small learnable projection."""
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor) -> Tensor:
        ...
        if self.config.sfp_freeze_clip:
            with torch.no_grad():
                text = self.text_encoder(input_ids=input_ids, attention_mask=attention_mask).pooler_output
        else:
            text = self.text_encoder(input_ids=input_ids, attention_mask=attention_mask).pooler_output
