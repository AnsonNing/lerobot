...
class StreamingFlowPolicy(PreTrainedPolicy):
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        ...
        chunk, final_action_state, _ = self._predict_action_chunk_and_state(batch, noise=noise)
    ...
    def select_action(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        ...
        if len(self._queues[ACTION]) == 0:
            actions, final_action_state, _ = self._predict_action_chunk_and_state(batch, noise=noise)
            self._prev_action_state = final_action_state
...
class StreamingFlowModel(nn.Module):
    ...
    def compute_loss(
        ...
        time = torch.rand(trajectory.shape[0], device=trajectory.device, dtype=torch.float32)
        ...
        pred_velocity = self.velocity_model(
            sample=noised_action.unsqueeze(1),
            timestep=time,
            global_cond=normalized_cond,
            freq=freq,
        )
