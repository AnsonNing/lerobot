...
def get_policy_class(name: str) -> type[PreTrainedPolicy]:
    ...
    if name == "tdmpc":
    ...
    elif name == "diffusion_v2":
        from .diffusion.modeling_diffusion_v2 import DiffusionV2Policy

        return DiffusionV2Policy
    elif name == "act":
    ...
    elif name == "smolvla_sfp":
        from .smolvla_sfp.modeling_smolvla_sfp import SmolVLASFPPolicy

        return SmolVLASFPPolicy
    elif name == "smolvla_sfp_v2":
        from .smolvla_sfp.modeling_smolvla_sfp_v2 import SmolVLASFPPolicy

        return SmolVLASFPPolicy
    elif name == "streaming_flow":
        from .streaming_flow.modeling_streaming_flow import StreamingFlowPolicy

        return StreamingFlowPolicy
    elif name == "streaming_flow_v2":
        from .streaming_flow.modeling_streaming_flow_v2 import StreamingFlowPolicy

        return StreamingFlowPolicy
    elif name == "streaming_flow_v3":
        from .streaming_flow.modeling_streaming_flow_v3 import StreamingFlowPolicy

        return StreamingFlowPolicy
    elif name == "streaming_flow_v4":
        from .streaming_flow.modeling_streaming_flow_v4 import StreamingFlowPolicy

        return StreamingFlowPolicy
    elif name == "streaming_flow_v5":
        ...
        return CropControlFlowPolicy
    elif name == "crop_controlflow_clip":
        from .crop_controlflow.modeling_crop_controlflow_clip import CropControlFlowClipPolicy

        return CropControlFlowClipPolicy
    elif name == "dispo":
        from .dispo.modeling_dispo import DiSPoPolicy

        return DiSPoPolicy
    elif name == "dispo_mamba3_flow":
        from .dispo.modeling_dispo_mamba3_flow import DiSPoMamba3FlowPolicy

        return DiSPoMamba3FlowPolicy
    elif name == "dispo_mamba3_flow_clip":
        from .dispo.modeling_dispo_mamba3_flow_clip import DiSPoMamba3FlowClipPolicy

        return DiSPoMamba3FlowClipPolicy
    elif name == "groot":
...
def make_policy_config(policy_type: str, **kwargs) -> PreTrainedConfig:
    ...
    if policy_type == "tdmpc":
    ...
    elif policy_type == "diffusion_v2":
        return DiffusionV2Config(**kwargs)
    ...
    elif policy_type == "smolvla_sfp":
        return SmolVLASFPConfig(**kwargs)
    elif policy_type == "smolvla_sfp_v2":
        return SmolVLASFPV2Config(**kwargs)
    elif policy_type == "streaming_flow":
        return StreamingFlowConfig(**kwargs)
    elif policy_type == "streaming_flow_v2":
        return StreamingFlowV2Config(**kwargs)
    elif policy_type == "streaming_flow_v3":
        return StreamingFlowV3Config(**kwargs)
    elif policy_type == "streaming_flow_v4":
        ...
        return CropControlFlowClipConfig(**kwargs)
    elif policy_type == "dispo":
        return DiSPoConfig(**kwargs)
    elif policy_type == "dispo_mamba3_flow":
        return DiSPoMamba3FlowConfig(**kwargs)
    elif policy_type == "dispo_mamba3_flow_clip":
        return DiSPoMamba3FlowClipConfig(**kwargs)
...
def make_pre_post_processors(
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    elif isinstance(policy_cfg, DiffusionV2Config):
        from .diffusion.processor_diffusion_v2 import make_diffusion_v2_pre_post_processors

        processors = make_diffusion_v2_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs.get("dataset_stats"),
        )
    ...
    elif isinstance(policy_cfg, SmolVLASFPConfig):
        from .smolvla_sfp.processor_smolvla_sfp import make_smolvla_sfp_pre_post_processors

        processors = make_smolvla_sfp_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs.get("dataset_stats"),
        )

    elif isinstance(policy_cfg, CropControlFlowConfig):
        from .crop_controlflow.processor_crop_controlflow import make_crop_controlflow_pre_post_processors

        processors = make_crop_controlflow_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs.get("dataset_stats"),
        )

    elif isinstance(policy_cfg, StreamingFlowConfig):
        from .streaming_flow.processor_streaming_flow import make_streaming_flow_pre_post_processors

        processors = make_streaming_flow_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs.get("dataset_stats"),
        )

    elif isinstance(policy_cfg, DiSPoConfig):
        from .dispo.processor_dispo import make_dispo_pre_post_processors

        processors = make_dispo_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs.get("dataset_stats"),
        )
