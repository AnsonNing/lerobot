...
@PreTrainedConfig.register_subclass("diffusion_v2")
@dataclass
class DiffusionV2Config(DiffusionConfig):
    """Diffusion Policy variant conditioned by frozen CLIP image and text encoders."""
    ...
    def __post_init__(self):
        ...
        if self.image_feature_dim <= 0:
            raise ValueError(f"`image_feature_dim` must be positive. Got {self.image_feature_dim}.")
        if self.text_projection_dim <= 0:
            raise ValueError(f"`text_projection_dim` must be positive. Got {self.text_projection_dim}.")
        if self.clip_image_crop_shape is not None and any(d <= 0 for d in self.clip_image_crop_shape):
            raise ValueError(
                f"`clip_image_crop_shape` must have positive dimensions. Got {self.clip_image_crop_shape}."
            )
